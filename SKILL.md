...
description: 对电力设备图像执行四阶段中文结构化标注：图像路由、设施/设备/部件树形标注、按载体动态筛选缺陷、确定性规则校验与大模型语义复核，并在门禁通过后生成多模态和组合检索描述。用于配电架空线路、当前已覆盖的变电设备图像、标签候选选择、标注JSON审核、候选树扩展和数据集构建；尤其适用于要求模型不能自由编造标签、需按业务域和对象路径渐进披露候选的任务。
---
