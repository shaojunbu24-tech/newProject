...
| 1.1.1 | `versions/1.1.1/` | 复制规则骨架后建立嵌套对象树，逐个载体标缺陷与证据，派生描述后运行确定性校验 | 复现1.0的树形升级版、强制可分辨实例逐个标注、不依赖候选标签树的受控流程 |

各版本分别维护。3.0新增双模式和双输出，但不会把参考模式的新词静默写入严格标签树；1.2、1.1.1和2.0也不会被3.0反向覆盖。1.1.1是1.0的树形结构升级版，不依赖候选标签树，强调对象必须嵌套、缺陷必须归属真实载体且可分辨绝缘子/套管逐个标注。
...
- 输电架空线路已开放绝缘子专项，可正式标注输电杆塔/线路通道下的绝缘子串、绝缘子类型、本体、釉表面、伞裙、端部金具及相关可见缺陷；输电其他对象、发电、用电及部分场站场景仍以路由能力为主。
...
│   └── self_test.py                  # 内置回归测试
└── versions/
    ├── 1.1.1/                        # 1.0树形升级版（对象嵌套+逐实例穷举）的独立可安装Skill
...
如需单独安装1.2，可先克隆仓库，再将版本目录复制为独立Skill：

```bash
git clone https://github.com/shaojunbu24-tech/newProject.git /tmp/power-image-annotation-repo
cp -a /tmp/power-image-annotation-repo/versions/1.2 \
  ~/.codex/skills/power-image-annotation-v1-2
```
...
```bash
cp -a /tmp/power-image-annotation-repo/versions/1.1.1 \
  ~/.codex/skills/power-image-annotation-v1-1-1
```

调用时使用`$power-image-annotation-v1-1-1`；1.1.1不依赖候选标签树，按`设施→设备→部件→子部件→缺陷`建立嵌套对象树，并要求可分辨绝缘子/套管逐个标注。
...
- 绝缘子缺少精确型号映射时回退到绝缘子候选，不继承父杆塔结构缺陷。
- “雷击”和“一般/严重”分别作为外部故障原因、外部业务等级；视觉标签只描述可见的放电、灼伤、闪络、污秽等现象。
